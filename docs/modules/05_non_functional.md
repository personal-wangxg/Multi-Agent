...
| 指标 | 目标值 | 说明 | 依据说明 |
|-----|--------|------|---------|
| 场景识别响应时间 | ≤ 5 秒 | 从用户提交到产出推荐结果 | 基于 9 类场景均为文本分类任务，不涉及复杂推理；同类产品参考值 |
| 配置生成响应时间 | ≤ 10 秒 | 完成初始 Agent 配置生成 | 模板驱动的配置生成，无需 LLM 深度推理，单模型调用可完成 |
| Agent 启动时间 | ≤ 30 秒 | 完成一次多 Agent 环境初始化 | 框架实例化 + Harness 加载的经验值 |
| 单会话并发 Agent 数 | ≥ 10 | 同时在线运行的 Agent 数量 | 虚拟教室场景中讨论式交互的最小支持多角色对话需求 |
| 运行稳定性 | ≥ 99% | 在正常运行时间内的任务成功率 | 教学辅助工具可用性的最低基准 |
| 记忆调取延迟 | ≤ 2 秒 | 从持久记忆检索一条相关信息的端到端延迟 | 向量检索 + 结构化查询混合模式的预期返回在本地部署可实现 |
| 单 Agent 输入上下文 Token 上限 | ≤ 模型窗口的 50% | 每个 Agent 每次调用 LLM 的输入 Token 不得超过模型上下文窗口的一半 | 防止上下文窗口溢出风险控制 |
| 全局上下文膨胀速率 | ≤ 5% / 轮 | 每轮 Agent 交互后，活动上下文 Token 增量不超过当前量的 5% | 与 FR-18 知识编译与 FR-15 Harness 约束的协同效果预期 |
| 跨会话记忆命中率 | ≥ 70% | 对重复出现的教学主题，能从持久记忆中检索到相关信息的比例 | 假设 LTM 混合检索（向量 + 关键词过滤可达到的召回率水平 |
| Harness 输出校验通过率 | ≥ 95% | Agent 首次输出即通过结构化校验的比例 | 模板约束与 YAML/JSON Schema 校验的期望成功率基准值 |
| Harness 校验延迟 | ≤ 500 ms | Harness 对单条 Agent 输出执行结构化/内容/权限校验的总耗时 | JSON Schema 纯本地校验无需 LLM 参与即可完成 |
| 未授权工具调用拦截率 | 100% | 不在白名单内的工具调用必须全部被 Harness 拦截，不得有漏网 | 安全红线，必须达到 100% |
...
| **长期记忆 LTM** | 向量数据库（**具体实现可选**，如 ChromaDB / PGVector / FAISS / Milvus 等） | 历史大纲、教案模板、教师反馈、成功配置快照、最佳 Agent 组合 | 永久保留（教师可手动删除） | 无硬性限制，按向量维度 × 数量估算；LTM 与决策索引相互独立，决策索引不作为向量片段存储 |
