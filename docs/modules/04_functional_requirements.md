...
> **阅读提示**：本文档功能需求按 FR 编号递增顺序排列（FR-01 → FR-18），建议使用文档大纲按编号跳转阅读。
...
| 不需要向量数据库 | 知识已编译为结构化页面和索引，查询时直接读取即可，不依赖向量相似度检索 |
...
#### 4.19.3 校验管道（Validation Pipeline）
